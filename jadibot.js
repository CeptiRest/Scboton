...
const jadibot = async (Cepti, m, from) => {
const { sendImage, sendMessage } = Cepti;
...
const Cepti = await makeWaSocket({
auth: state,
browser: [`Jadibot Md By (Cepti Bot)`, "Chrome", "1.0.0"],
logger: log({ level: "silent" }),
version,
})

Cepti.ws.on('CB:Blocklist', json => {
...
let qutsnya = await Cepti.sendContact(callerId, owner)
await Cepti.sendMessage(callerId, { text: `Sistem Otomatis Block!!!\nJangan Menelpon Bot!!!\nSilahkan Hubungi Owner Untuk Dibuka!!!`}, { quoted : qutsnya })
await sleep(8000)
...
if (!Cepti.public && !kay.key.fromMe && chatUpdate.type === 'notify') return
if (kay.key.id.startsWith('BAE5') && kay.key.id.length === 16) return
m = smsg(Cepti, kay, store)
...
store.bind(Cepti.ev);
Cepti.ev.on("creds.update", saveCreds);
Cepti.ev.on("connection.update", async up => {
...
if (connection == "open") {
Cepti.id = Cepti.decodeJid(Cepti.user.id)
Cepti.time = Date.now()
global.conns.push(Cepti)
await m.reply(`*Connected to Whatsapp - Bot*\n\n*User :*\n _*× id : ${Cepti.decodeJid(Cepti.user.id)}*_`)
user = `${Cepti.decodeJid(Cepti.user.id)}`
...
console.log(`Bad Session File, Please Delete Session and Scan Again`); Cepti.logout(); }
...
console.log("Connection Replaced, Another New Session Opened, Please Close Current Session First"); Cepti.logout(); }
else if (reason === DisconnectReason.loggedOut) { 
console.log(`Device Logged Out, Please Scan Again And Run.`); Cepti.logout(); }
else if (reason === DisconnectReason.restartRequired) { 
console.log("Restart Required, Restarting..."); start(); }
else if (reason === DisconnectReason.timedOut) { 
console.log("Connection TimedOut, Reconnecting..."); start(); }
else Cepti.end(`Unknown DisconnectReason: ${reason}|${connection}`)
}
})
...
FN:${await Cepti.getName(i + '@s.whatsapp.net')}\n
item1.TEL;waid=${i}:${i}\n
